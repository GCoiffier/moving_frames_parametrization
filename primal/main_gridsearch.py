...
if __name__ == "__main__":
    ...
    if options.distortion == Distortion.NONE:
        options.n_iter_max = 300

    options.set_schedule() # default schedule depending on distortion

    WEIGHTS = [(_w, 1.) for _w in np.logspace(1,3,20)][::-1] + [(1., _w) for _w in np.logspace(0, 3, 20)]
    with open( os.path.join(FOLDER, f"report.csv"), 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=["Edge_weight", "FF_weight", "energy", "time", "n_singus", "conformal", "authalic", "det", "iso", "shear", "stretch_max", "stretch_mean"])
        writer.writeheader()
        
        for e_weight, ff_weight in WEIGHTS:
                print(e_weight, ff_weight)
                try:
                    ###### Initialization ###### 
                    mesh = M.mesh.load(args.input_mesh)
                    subfolder = os.path.join(FOLDER, f"E{e_weight:.2E}_FF{ff_weight:.2E}")
                    ...
                    # Distortion
                    distMeasure = M.processing.SurfaceDistortion(instance.mesh)()
                    ...
                    M.mesh.save(flat_param, os.path.join(subfolder,"param_flat.geogram_ascii")) # for attributes
