...
@jit(nopython=True, parallel=True, cache=True)
def constraint_rotations_follow_ff_noJ(X : np.ndarray, I : np.ndarray, PT : np.ndarray, n):
    """Rotations along edges should match the rotation between the two frames"""
    val  = np.zeros(2*n , dtype=np.float64)
    for i in prange(n):
        iw = I[i,0]
        iza, izb = I[i,1], I[i,2]
...
@jit(nopython=True, parallel=True, cache=True)
def constraint_rotations_follow_ff(X : np.ndarray, I : np.ndarray, PT : np.ndarray, n):
    """Rotations along edges should match the rotation between the two frames"""
    val  = np.zeros(2*n , dtype=np.float64)
...
def distortion_isometric_noJ(X : np.ndarray, I : np.ndarray, jacs : np.ndarray):
    ...
    for i in prange(n):
        ...
        u0,v0 = X[ie0], X[ie0+1]
        u1,v1 = X[ie1], X[ie1+1]
        a,b,c = ref_mat[0,0], ref_mat[0,1], ref_mat[1,1]
        val[3*i] = a*u0*u0+ 2*b*u0*v0 + c*v0*v0 - 1
        val[3*i+1] = a*u1*u1+ 2*b*u1*v1 + c*v1*v1 - 1
        val[3*i+2] = 2*(a*u0*u1 + c*v0*v1 + b*(u0*v1 + u1*v0))
    return val
...
def distortion_isometric(X : np.ndarray, I : np.ndarray, jacs : np.ndarray):
    ...
    for i in prange(n):
        ...
        val[3*i] = a*u0*u0+ 2*b*u0*v0 + c*v0*v0 - 1
        rowJ[8*i],   colJ[8*i],   valJ[8*i]   = 3*i, ie0,   2*a*u0 + 2*b*v0
        rowJ[8*i+1], colJ[8*i+1], valJ[8*i+1] = 3*i, ie0+1, 2*c*v0 + 2*b*u0

        val[3*i+1] = a*u1*u1+ 2*b*u1*v1 + c*v1*v1 - 1
        rowJ[8*i+2], colJ[8*i+2], valJ[8*i+2] = 3*i+1, ie1,   2*a*u1 + 2*b*v1
        rowJ[8*i+3], colJ[8*i+3], valJ[8*i+3] = 3*i+1, ie1+1, 2*c*v1 + 2*b*u1
        
        val[3*i+2] = 2*(a*u0*u1 + c*v0*v1 + b*(u0*v1 + u1*v0))
        rowJ[8*i+4], colJ[8*i+4], valJ[8*i+4] = 3*i+2, ie0,   2*(a*u1 + b*v1)
        rowJ[8*i+5], colJ[8*i+5], valJ[8*i+5] = 3*i+2, ie0+1, 2*(c*v1 + b*u1)
        rowJ[8*i+6], colJ[8*i+6], valJ[8*i+6] = 3*i+2, ie1,   2*(a*u0 + b*v0)
        rowJ[8*i+7], colJ[8*i+7], valJ[8*i+7] = 3*i+2, ie1+1, 2*(c*v0 + b*u0)
    return val,valJ,rowJ,colJ
...
def distortion_shear_noJ(X : np.ndarray, I : np.ndarray, jacs : np.ndarray):
    ...
    for i in prange(n):
        ...
        val[i] = a*u0*u1 + c*v0*v1 + b*(u0*v1 + u1*v0)
    return val
...
def distortion_shear(X : np.ndarray, I : np.ndarray, jacs : np.ndarray):
    ...
    for i in prange(n):
        ...
        val[i] = a*u0*u1 + c*v0*v1 + b*(u0*v1 + u1*v0)
        rowJ[4*i],   colJ[4*i],   valJ[4*i]   = i, ie0,   a*u1 + b*v1
        rowJ[4*i+1], colJ[4*i+1], valJ[4*i+1] = i, ie0+1, c*v1 + b*u1
        rowJ[4*i+2], colJ[4*i+2], valJ[4*i+2] = i, ie1,   a*u0 + b*v0
        rowJ[4*i+3], colJ[4*i+3], valJ[4*i+3] = i, ie1+1, c*v0 + b*u0
    return val,valJ,rowJ,colJ
