...
def init_attributes(I : Instance):
    ...
    I.barycenters = M.attributes.face_barycenter(I.mesh, persistent=False)
    I.areas = M.attributes.face_area(I.mesh, persistent=False)
    
    M.attributes.face_normals(I.mesh) # initialize face normals (used several times afterwards by other attributes)
    I.angles = M.attributes.corner_angles(I.mesh, persistent=False)
    I.vnormals = M.attributes.vertex_normals(I.mesh)
    
    # build defects from angles: 
    I.defect = I.mesh.vertices.create_attribute("defect", float, dense=True)
    for C,V in enumerate(I.mesh.face_corners):
        I.defect[V] += I.angles[C]

def init_local_bases(I : Instance):
    """ Initializes natural parallel transport """
    NV = len(I.mesh.vertices)
    ...
    for u in I.mesh.id_vertices:
        ...
        P = M.Vec(I.mesh.vertices[u])
        N = I.vnormals[u]

        # extract basis edge
        v = I.mesh.connectivity.vertex_to_vertex(u)[0]
        E = I.mesh.vertices[v] - P
        ...
        for v in I.mesh.connectivity.vertex_to_vertex(u):
            T,iu,_ = I.mesh.half_edges.adj(u,v)
            if bnd:
                I.parallel_transport[(u,v)] = ang * pi / I.defect[u]
            else:
    ...
    I.PT_array = np.zeros(len(I.mesh.edges), dtype=np.float64)
    for e,(A,B) in enumerate(I.mesh.edges):
        I.PT_array[e] = principal_angle(I.parallel_transport[(B,A)] - I.parallel_transport[(A,B)])

    # compute curvature on the mesh
    if I.mesh.faces.has_attribute("curvature"):
        I.curvature = I.mesh.faces.get_attribute("curvature")
    else:
        I.curvature = I.mesh.faces.create_attribute("curvature", float)
        for iF,F in enumerate(I.mesh.faces):
            v = 1+0j
            for a,b in cyclic_pairs(F):
...
def init_variables_edges(I : Instance):
    ...
    I.var_sep_pt = 4*len(I.mesh.edges)
    n_edge_var = 4*len(I.mesh.edges) + 6*len(I.mesh.faces)
    I.nvar += n_edge_var
    var_edge = np.zeros(n_edge_var, dtype=np.float64)
    EL = M.attributes.edge_length(I.mesh, persistent=False) # Edge Lengths
    I.edge_lengths = -1*np.ones((len(I.mesh.edges), 3), dtype=np.float64)

    for u, Pu in enumerate(I.mesh.vertices):
        edges_u = I.mesh.connectivity.vertex_to_edge(u) # the edges around u
        for ie in edges_u:
            e = I.mesh.edges[ie]
            direct = (e[0]==u) # edge is (u,v) or (v,u)
            v = e[1] if direct else e[0] # other extremity of edge
            Pv = I.mesh.vertices[v]
            elen = EL[ie]/2
            ...
            bnd = I.mesh.is_vertex_on_border(u)
            for iw,w in enumerate((I.mesh.half_edges.next_around(u,v), I.mesh.half_edges.prev_around(u,v))):
                if w is not None:
                    T = I.mesh.connectivity.face_id(u,v,w)
                    iuT = I.mesh.connectivity.in_face_index(T,u)
                    Pbary = I.barycenters[T]
                    slen = geom.distance(Pbary, Pu)
                    sangle = geom.angle_3pts(Pv, Pu, Pbary)
                    sangle *= 2*pi/ I.defect[u] if not bnd else pi / I.defect[u]
...
def init_var_ff_and_rot(I : Instance):
    ...
    for j,i in tree.traverse():
        ...
        aij, aji = I.parallel_transport[(i,j)], I.parallel_transport[(j,i)]
...
def init_distortion_matrices(I : Instance):
    ...
    for (iS, ie1, ie2) in I.quads:
        ...
        m1 = np.array([[ue1, ue2], 
                       [ve1, ve2]])
        m1 = np.linalg.inv(m1)
        I.dist_matrices.append(m1)
        I.dist_matrices_squared.append(m1.T @ m1)

        m2 = np.array([[uS - ue1, uS - ue2], 
                       [vS - ve1, vS - ve2]])
        m2 = np.linalg.inv(m2)
        I.dist_matrices.append(m2)
        I.dist_matrices_squared.append(m2.T @ m2)

    I.dist_matrices = np.array(I.dist_matrices)
    I.dist_matrices_squared = np.array(I.dist_matrices_squared)
...
def init_edge_indices(I : Instance):
    ...
    len_i = 0
    for ie,(A,B) in enumerate(I.mesh.edges):
    ...
    for T in I.mesh.id_faces:
        for ie in I.mesh.connectivity.face_to_edge(T):
            A,B = I.mesh.edges[ie]
            iA = I.mesh.connectivity.in_face_index(T,A)
            iB = I.mesh.connectivity.in_face_index(T,B)
            I.edge_indices[ind_i, 0] = ie
            I.edge_indices[ind_i, 1] = I.var_sep_pt + 6*T  + 2*iA
            I.edge_indices[ind_i, 2] = I.var_sep_pt + 6*T  + 2*iB
            if I.mesh.half_edges.adj(A,B)[0]==T:
...
class Initializer(Worker):
    ...
    def initialize(self):
        ...
        var_ff, var_rot = init_var_ff_and_rot(self.instance)
        ...
        self.log("Compute initial distortion matrices")
        if self.options.distortion!= Distortion.NONE:
