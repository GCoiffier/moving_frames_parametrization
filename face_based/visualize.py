...
def generate_visualization(mesh_name):
    mesh_path = f"output/{mesh_name}/{mesh_name}.obj"
    mesh = M.mesh.load(mesh_path)

    singus, seams, feat = import_custom_fields(f"output/{mesh_name}/{mesh_name}.obj") # parse additionnal data
    
    seamsMesh = get_seams(seams,mesh)
    #seamsMesh = M.procedural.cylindrify_edges(seamsMesh, N=30, radius=0.01)
    M.mesh.save(seamsMesh, f"output/{mesh_name}/visu_seams.obj")

    featMesh = get_seams(feat, mesh)
    #featMesh = M.procedural.cylindrify_edges(featMesh, N=30, radius=0.01)
    M.mesh.save(featMesh, f"output/{mesh_name}/visu_feats.obj")
    ...
    for PC,which in ((singuPos, "pos"), (singuNeg, "neg"), (singuCorner,"corner")):
        ...
        M.mesh.save(PC, f"output/{mesh_name}/visu_singu_{which}.obj")
