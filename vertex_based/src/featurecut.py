...
class FeatureCutter(M.processing.FeatureEdgeDetector):
    ...
    def run(self):
        ...
        self.cut_mesh = M.mesh.RawMeshData( M.mesh.copy(self.input_mesh, copy_attributes=True, copy_connectivity=False))
    ...
    def cut(self):
        ...
        self.ref_vertex = dict([(v, None) for v in self.input_mesh.id_vertices]) # copied vertices id -> original vertex id
        ...
        # duplicate every feature vertex by creating degree-1 copies
        for ref_v in self.feature_vertices:
            ...
            for _ in range(ring_degrees[ref_v]-1):
                # create a copy of ref_v and fill containers
        ...
        # split rings of vertices into ring_degree[v] parts
        for ref_v in self.feature_vertices:
            iring = 0
            deg = ring_degrees[ref_v]
            new_v = self.cut_vertices[ref_v][iring]
            nF = len(self.input_mesh.connectivity.vertex_to_faces(ref_v))
            for iF, F in enumerate(self.input_mesh.connectivity.vertex_to_faces(ref_v)): # self.input_mesh instead of cutmesh so we do not recompute connectivity data just for this
                if (iF-1)%nF in self.local_feat_edges[ref_v]:
                    iring += 1
                    new_v = self.cut_vertices[ref_v][iring%deg]
                self.cut_mesh.faces[F] = replace_in_list(self.cut_mesh.faces[F], ref_v, new_v) # replace v by iv in the face

        # recompute corners and edges
